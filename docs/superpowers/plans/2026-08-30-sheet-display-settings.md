...
**移設で挙動が1つ変わる。** 今は結果が無いとき `showMapState()` が `#zoomCtl` ごと
隠すので矢印・角丸のボタンも消えるが、設定タブへ移すと常に出る。改善方向の変化だが、
レビューで「意図した変化」と分かるようにここに書いておく。
...
5. 端数「半」の切替が設定タブの2階層奥へ移って不便でないか。
   これは紙の中身を変える設定（P数欄が `25P 半2` になる）なので、
   毎日の動線から遠のいてよいかを現物で確かめる
6. 初期値（品名16 / ロット13 / P数16 / 注釈13px）を確定する。
   変えるなら Task 7 へ。変えないならその旨を記録して Task 7 はスキップ
