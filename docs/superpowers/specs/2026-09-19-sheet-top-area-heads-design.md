...
- グループが1つだけで、そのグループキーが `sheetAreas("top")[0]`（現在は軒下①）に一致し、
  **かつそのグループが持つエリアが1つだけ**（＝またがる欄が混じっていない）
- 上段に荷物が1つも無い（グループ0）

この2つに当てはまる日は、見出し1セル・固定文字「軒下」・`colspan` は `lay.top*2`。
従来の紙と同じ見た目になる。日常の大半はここに落ちる。

1つ目の条件の「エリアが1つだけ」は、下の表と食い違わせないために要る。
これが無いと、軒下①と軒下②にまたがる欄1件だけの日がグループキー軒下①の1グループに
なって固定文字「軒下」に落ち、表が決めている `軒下①・軒下②` と矛盾する。
実装は `topHeadGroups()` の `groups[0].names.length===1`（`files/index.html:4537`）。

なおこの条件を外しても、またがる欄のエリアが紙から消えるわけではない。
§3-5 の注釈が `※軒下②` を欄の真下に出す。どちらで読ませるかの取り決めの問題で、
表は「見出しからも読める」ほうを選んでいる。
...
| `軒下①・出庫口横 ほか`（打ち切り後） | 188px | 0.49 | **0.39** |
| `出庫口横・5棟壁際 ほか`（打ち切り後） | 202px | 0.46 | **0.37** |

**1欄ぶんの幅で危ないのは3エリア以上を連結したとき**。wide で 0.4 を割って折り返し、
見出し行が約18px 伸びる。§3-3 で先頭2つ＋「 ほか」に打ち切るのはこのため。

ただし**打ち切っても wide の1欄ぶんでは 0.4 を割る**（上の表の下2行）。
` ほか` の分が 44px あり、`軒下①・出庫口横` の 144px に足すと 188px になるため。
つまり打ち切りは normal（0.49 で安全）では効くが、wide では折り返しを防げない。
それでも折り返しに落とすほうを選んでいる理由は変わらない（§4-5 の警告で気づけて、
印刷の高さにも余裕がある。wide の実測 661.2px に対し上限 718.5px）。
打ち切り記号を ` ほか` から `…` のような短いものに変えれば wide でも 0.4 を超える見込みだが、
現場が読む文字としてどちらが良いかは別の判断なので、ここでは変えていない。
