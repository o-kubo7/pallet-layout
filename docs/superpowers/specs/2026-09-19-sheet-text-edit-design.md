...
測定環境: サンプル伝票9件に、同じ品名で3ロットずつの品目を3品（部品Z・部品Y・部品W）と
追加の荷物4件を足し、伝票22件・荷物22件にした状態。様式 wide（上段6欄・下段9欄）で
追記欄が出て、まとめ欄（`.roll`、`font-size:55%`、`files/index.html:515`）が4個ある。
この時点では1ページ超過の警告は出ていない。測定は Chrome、`zoom` を補正した実寸。
...
まとめ欄の行数を増やしたときの、表全体の伸び（base からの差分）:
...
3行までは表の高さが変わらない。4行から伸び始め、6行で +22.0px。この構成では
余裕 43.0px に対して6行でもまだ1ページに収まる。

ただし、この 470.2px / 43.0px は**この構成での1例**であり、固定の上限値としては
使えない。表の高さ・余裕は荷物の件数・文字サイズ・様式によって変わる。既存の教訓
（2026-09-18、「この指定を当てた状態で5件までは1ページに収まる」）と食い違うわけでは
なく、どちらも自分の測定条件の範囲でのみ成り立つ値である。

画面表示では `.fit` の実効 font-size は 8.8px（`font-size:55%` の指定に
`fitSheetText()` の圧縮がかかった値）。編集で開くと `<textarea>`（`rows=3`）に変わり、
font-size 16px・`transform:none` になるため、8.8px の表示に対して 16px で打てる。
§3-9 で懸念した「7px では打てない」は、編集中の font-size 下限 13px によって
解消している（実測の 16px はその下限より上）。

開いたときの `el.value` は `td.dataset.auto` と完全に一致する。まとめ欄でも、
編集していない欄が書き足しとして保存される「幽霊 override」は起きない。
...
- 追記欄を出す日を作り、まとめ欄（`font-size:55%`）を編集できるか、
  行を増やしたときの伸びが §6-6 の実測値のとおりかを確かめる
