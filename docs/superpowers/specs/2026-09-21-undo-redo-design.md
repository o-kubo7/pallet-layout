...
`.toolflag` は `flex-wrap:wrap` なので、基準幅が `auto` だと説明文が縮む前に折り返し、帯が 2 行以上になる。

375px 幅での実測（帯の使える幅は 318px、padding 12px×2 と gap 10px×2 を含む）。

| 状態 | 帯の高さ | 戻すボタンの左端 | 進むボタンの右端 |
|---|---|---|---|
| 選択なし・案内なし | 62px | 188px | 338px |
| 選択あり・案内なし | 62px | 188px | 338px |
| 選択なし・案内あり | 62px | 188px | 338px |
| 選択あり・案内あり | 62px | 188px | 338px |
| 運搬中 | 62px | 188px | 338px |

記号のみのときは左端が 240px になる（ボタンが短くなるぶん）。どの状態でも右端は 338px で不動。
