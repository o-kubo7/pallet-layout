...
**2026-09-21 訂正。** `growStashCol()` は `2026-09-21-stash-whole-space-move-design.md` で
削除された。退避への移動はスペース全体が宛先になり、`applyMoveToStash()` が
`h:STASH_COL_H` の列を必要な分だけ追加するだけなので、上のコード例と「列をその場で
伸ばす」注記はもう起きない。早めに `sp` を控える必要そのものは undo の設計として今も成り立つ。
...
- `applyMove()` が途中で中止したときに、`growStashCol()` で伸びた退避スペースの列を戻すこと。
  ...
  直すのは別の作業として切り分ける
  （**2026-09-21 訂正。** `growStashCol()` は `2026-09-21-stash-whole-space-move-design.md` で
  削除されたため、この不具合自体がもう存在しない。退避への移動はスペース全体が宛先になり、
  `applyMoveToStash()` は `h:STASH_COL_H` の列を追加するだけで、途中で中止しても伸びた列が
  残るという経路がない）
