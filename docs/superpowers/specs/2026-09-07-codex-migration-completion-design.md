...
- `novel` は `AGENTS.md`、5つのカスタムエージェント、2つのHooksが移行済み。調査開始時点ではGit管理されていない。
...
`novel` はユーザーの追加判断により、設定変更前にGit管理を開始する。

- `.gitignore` では `.DS_Store` だけを除外する。
- `.claude/`、`.codex/`、原稿、canon、既存レビュー、`.canon.sha`、既存のバックアップファイルを含む現在のプロジェクト状態を初回コミットする。
- ブランチ名は `main` とする。
- remoteは設定せず、pushや公開は行わない。
- 初回コミットの後、5つのCodexエージェントへのモデル設定を別コミットにする。これにより移行前後をGitで比較・復元できるようにする。
...
- `novel` は設定変更前の全状態を初回コミットし、その後の変更を別コミットにして復元可能にする。
...
- `main` ブランチでGit管理され、remoteが設定されていない。
- 初回コミットとモデル設定コミットが分かれている。
- `.DS_Store` が追跡されていない。
