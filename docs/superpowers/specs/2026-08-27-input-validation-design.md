...
**6. SNP だけが入った行は黙ってスキップされる。**
品名を入れて消すと `nameChanged()` が自動補完した SNP が残る。この行は空行として扱い、
赤枠も出さず配置もしない。品名・ロット・個数のいずれも無い行は配置に必要な情報を
持たないため実害は無いが、「SNP だけを手で打った行」も同じように無視される。
ロットか個数が1文字でも入っていれば今までどおり不完全な行として捕まる。
