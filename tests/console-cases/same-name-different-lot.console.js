// 同名・別ロット (same-name-different-lot) — verified
// 現在の時間帯の入力と配置関連状態（配置不可設定を含む）をリセットします。
// 反対側の時間帯と品目マスタは変更しません。
// 専用オリジンまたは不要な入力の時間帯で実行してください。
(() => {
  const rows = [{"type":"充填品","name":"部品1","lot":"S-101","snp":10,"qty":10},{"type":"充填品","name":"部品1","lot":"S-102","snp":10,"qty":10}];
  const manualChecks = ["別ロットが配置図で別欄として表示されるか確認する","合計2Pが表示されるか確認する"];
  ...
  console.info("入力完了: " + entered.length + "件 / " + totalP + "P（期待 2P）");
